            ...
            <div class="dop">
              ...
              <div class="price-quantity-row">
                ...
                <div class="quantity-control">
                  ...
                  <input type="number" class="quantity-input" value="${existingCartItem ? existingCartItem.quantity : quantity}" min="1" max="${stock}">
    ...
    confirmDeliveryButton.addEventListener('click', () => {
      ...
      if (existingCartItem) {
        const productData = {
          ...
          basePrice: basePrice,
          comision: parseFloat(product.comision) || 0,
          deliveryCost: calculateDeliveryCost(product.precio),
          image: images[0],
          details: product.descripcion,
          quantity: existingCartItem.quantity,
          stock: stock,
...
export function updateCartDisplay(cartItems, modalBodyCarrito) {
  ...
  const total = cartItems.reduce((sum, item) => {
    ...
    return sum + (priceNum * item.quantity + deliveryCost);
  }, 0);

  cartItems.forEach(item => {
    const priceNum = parseFloat(item.price) || 0;
    const deliveryCost = parseFloat(item.deliveryCost) || 0;
    const subtotal = priceNum * item.quantity + deliveryCost;
