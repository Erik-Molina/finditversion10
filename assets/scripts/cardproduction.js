...
function sanitizeProductNameForUrl(name) {
  ...
  // Convertir a minúsculas, reemplazar espacios y caracteres no válidos, preservar acentos
...
function sanitizeProductNameForMatch(name) {
  if (typeof name !== 'string') return '';
  // Igual que sanitizeProductNameForUrl pero sin encodeURIComponent
...
function addToCart(productData, updateOnly = false) {
  ...
  if (existingItem) {
    if (updateOnly) {
      ...
      existingItem.deliveryCost = productData.deliveryCost;
    } else {
      // Update quantity and delivery fields
...
function handleProductHash() {
  ...
  if (productMatch) {
    ...
    let productName;
    try {
      productName = decodeURIComponent(productMatch[3]); // Decode URL-encoded name
    } catch (err) {
